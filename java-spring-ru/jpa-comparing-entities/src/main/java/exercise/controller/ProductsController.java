...
public class ProductsController {
    ...
    public ResponseEntity<Product> create(@RequestBody Product productData) {
        List<Product> products = productRepository.findAll().stream().filter(product -> product.equals(productData))
                .toList();
        if (products.isEmpty()) {
            productRepository.save(productData);
            return ResponseEntity.status(HttpStatus.CREATED).body(productData);
        } else {
            throw new ResourceAlreadyExistsException("Product with id  "  + productData.getId()  +  " already exists");
        }
    }
    // END
